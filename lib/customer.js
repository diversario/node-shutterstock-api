...
Customer.prototype.lightboxes = function(next) {
	...
	var self = this;
	if (this._lightboxes) {
		var lightboxes = clone(this._lightboxes);
		next(null, lightboxes);
		return new Q(lightboxes);
	} else {
		return self._client.get({
...
Customer.prototype.findSubscriptions = function(criteria, next) {
	...
	this.subscriptions(function(err, subscriptions, res) {
		if (err) {
			...
			result = und.where(subscriptions, criteria);
			next(null, result, res);
		}
	});
	return error ? Q.reject(error) : new Q(result);
};
...
Customer.prototype.licenseVideo = function(id, options, next) {
	...
	this.findSubscriptions(options.subscription, function(err, subscriptions) {
		if (subscriptions.length !== 1) {
			var error = new Error('No subscriptions matching ' + JSON.stringify(options.subscription)); 
			next(error);
		...
		if (s.sizesForLicensing.filter(function(size) { return size === options.size; }).length === 0) {
			var sizeError = new Error('Invalid size');
			next(sizeError);
			return Q.reject(sizeError);
		}
...
Customer.prototype.licenseImage = function(id, options, next) {
	...
	this.findSubscriptions(options.subscription, function(err, subscriptions) {
		if (subscriptions.length !== 1) {
			var lengthError = new Error('No subscriptions matching ' + JSON.stringify(options.subscription)); 
			next(lengthError);
			return Q.reject(lengthError);
		}
